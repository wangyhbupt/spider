...
def CreateDriver():
    ...
    driver = webdriver.PhantomJS(desired_capabilities=desire,service_args=['--load-images=no'])
...
def CheckMatchEnd(driver):
    match_report_node = driver.find_element_by_xpath("//a[contains(text(), 'Match Report')]")
